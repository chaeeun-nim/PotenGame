// src/components/List-ItemDetail/ItemDetailImage.tsx
...
export default function ItemDetailImage({
  ...
  // 상품 상세 이미지 URL 생성 (productData 우선 사용)
  const getDetailImageUrl = () => {
    // productData에서 상세 이미지 호출
    if (productData?.content?.[0]?.path) {
      return productData.content[0].path;
    }
    // fallback: productId 활용
    if (productId) {
  ...
  const getImageAlt = () => {
    if (productData?.name) {
      return `${productData.name} 상세 이미지`;
    }
    return `${category || '상품'} 상세 이미지`;
  };

  const detailImageUrl = getDetailImageUrl();
